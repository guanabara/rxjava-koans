...
public class lessonC_BooleanLogicAndErrorHandling {
    ...
    public void _1_takeWhileEvaluatesAnExpressionAndEmitsEventsUntilItReturnsFalse() {
        ...
        Func1<ElevatorPassenger, Boolean> elevatorRule = passenger -> passenger.mWeightInPounds + elevator.getTotalWeightInPounds() < Elevator.MAX_CAPACITY_POUNDS;
        /**
         ...
         */
        mSubscriber = new TestSubscriber<>();
        TestSubscriber<ElevatorPassenger> passengerSubscriber = new TestSubscriber<>();

        elevator.unload();

        elevatorQueueOne.takeWhile(elevatorRule).doOnNext(elevator::addPassenger).subscribe(passengerSubscriber);
    ...
    public void _4_challenge_compositionMeansTheSumIsGreaterThanTheParts() {
        ...
        MathObservable.sumInteger(range.filter(el -> el >= 9)).subscribe(sum -> mSum = sum);
    ...
    public void _6_retryCanAttemptAnOperationWhichFailsMultipleTimesInTheHopesThatItMaySucceeed() {
        Observable<String> networkRequestObservable = Observable.just(new ComcastNetworkAdapter()).map(networkAdapter -> networkAdapter.getData().get(0)).repeat(100);
        networkRequestObservable.retry(48).subscribe(mSubscriber);
    ...
    public void _7_combineLatestTakesTheLastEventsOfASetOfObservablesAndCombinesThem() {
        ...
        Func3<Boolean, Boolean, Boolean, Boolean> combineTumblerStatesFunction = (tumblerOneUp, tumblerTwoUp, tumblerThreeUp) -> (Boolean) (tumblerOneUp && tumblerTwoUp && tumblerThreeUp);
