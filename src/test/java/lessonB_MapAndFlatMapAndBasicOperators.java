...
public class lessonB_MapAndFlatMapAndBasicOperators {
    ...
    public void _2_flatMapUnwrapsOneLevelOfNestingInAnObservableStream() {
         ...
         */
        Observable<Observable<CarnivalFood>> map = foodCartItemsObservable.map(foods -> {
         ...
         */
        Observable<CarnivalFood> individualItemsObservable = foodCartItemsObservable.flatMap((Func1<List<CarnivalFood>, Observable<CarnivalFood>>) Observable::from);
        individualItemsObservable.subscribe(mSubscriber);
        assertThat(mSubscriber.getOnNextEvents()).hasSize(chineseFoodCart.size() + funnelCakeCart.size());
         ...
         */
        individualItemsObservable.filter(food -> food.mPrice < 5.00).subscribe(mSubscriber);
    ...
    public void _3_theReduceOperatorAccumulatesValuesAndEmitsTheResult() {
        ...
        assertThat(testSubscriber.getOnNextEvents().get(0)).isEqualTo(elevatorPassengers.stream().map(ElevatorPassenger::getWeightInPounds).reduce(0, (x,y) -> x+y));
    }
